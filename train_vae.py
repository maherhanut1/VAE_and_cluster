...
parser.add_argument("--feature_scale", "-fs", help="Feature loss scale", type=float, default=16)
...
for epoch in trange(start_epoch, args.nepoch, leave=False):
    vae_net.train()
    vae_net(torch.normal(0, 1, size=(32, 3, 128, 128)))
        ...
        if (current_iter + 1) % args.save_interval == 0:
            ...
            with torch.no_grad():
                with torch.cuda.amp.autocast():
                    ...
                    recon_img = vae_net(test_images.to(device))
